import { StyleSheet } from 'react-native';
import { theme } from './theme';

// Browse and map surfaces, filters, chips, result cards, and marker styles.
export const browseStyles = {
  browseHeaderCard: {
    backgroundColor: 'rgba(20, 20, 24, 0.94)',
    borderColor: theme.borderStrong,
    borderRadius: 28,
    borderWidth: 1,
    gap: 10,
    padding: 12,
    shadowColor: '#000000',
    shadowOffset: { width: 0, height: 14 },
    shadowOpacity: 0.22,
    shadowRadius: 24,
    elevation: 18,
  },
  browseHeaderCardLandscape: {
    alignSelf: 'center',
    gap: 6,
    maxWidth: 620,
    padding: 10,
  },
  toolbarRow: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 6,
    justifyContent: 'space-between',
  },
  toolbarRowLeading: {
    justifyContent: 'flex-start',
  },
  toolbarActionsRow: {
    alignItems: 'center',
    flexDirection: 'row',
    flexShrink: 1,
    gap: 4,
    justifyContent: 'flex-start',
  },
  toolbarActionsRowFill: {
    flex: 1,
  },
  toolbarActionsRowWithDashboard: {
    flex: 1,
    minWidth: 0,
  },
  toolbarRowLandscape: {
    minHeight: 36,
  },
  browseModeContentLayer: {
    flex: 1,
  },
  browseContentStage: {
    flex: 1,
    position: 'relative',
  },
  browseContentFill: {
    ...StyleSheet.absoluteFillObject,
  },
  appTitle: {
    color: theme.textPrimary,
    fontSize: 24,
    fontWeight: '800',
  },
  controlsTitleLandscape: {
    fontSize: 20,
  },
  mapScreen: {
    flex: 1,
  },
  mapModeContentLayer: {
    ...StyleSheet.absoluteFillObject,
  },
  mapThemeTransitionLayer: {
    ...StyleSheet.absoluteFillObject,
  },
  mapBackground: {
    ...StyleSheet.absoluteFillObject,
  },
  mapOverlayLayer: {
    ...StyleSheet.absoluteFillObject,
    gap: 12,
    justifyContent: 'flex-start',
    paddingHorizontal: 18,
    paddingTop: 0,
    paddingBottom: 0,
  },
  mapOverlayContentLayer: {
    gap: 12,
  },
  mapTopPanel: {
    backgroundColor: 'rgba(20, 20, 24, 0.9)',
    borderColor: theme.borderStrong,
    borderRadius: 28,
    borderWidth: 1,
    gap: 10,
    padding: 12,
    shadowColor: '#000000',
    shadowOffset: { width: 0, height: 14 },
    shadowOpacity: 0.2,
    shadowRadius: 24,
    elevation: 16,
  },
  mapTopPanelLandscape: {
    alignSelf: 'center',
    gap: 6,
    maxWidth: 620,
    padding: 10,
  },
  mapLoadingOverlay: {
    alignItems: 'center',
    alignSelf: 'center',
    backgroundColor: 'rgba(18, 22, 30, 0.96)',
    borderRadius: 20,
    gap: 10,
    paddingHorizontal: 20,
    paddingVertical: 18,
  },
  mapPreviewCard: {
    backgroundColor: 'rgba(23, 28, 37, 0.97)',
    borderColor: theme.border,
    borderRadius: 24,
    borderWidth: 1,
    gap: 14,
    padding: 16,
  },
  mapPreviewCardLandscape: {
    alignSelf: 'center',
    gap: 10,
    maxWidth: 320,
    padding: 12,
    width: '100%',
  },
  mapResultsCard: {
    backgroundColor: 'rgba(23, 28, 37, 0.97)',
    borderColor: theme.border,
    borderRadius: 24,
    borderWidth: 1,
    gap: 12,
    minHeight: 0,
    padding: 16,
  },
  mapResultsHeader: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  mapResultsHeaderCopy: {
    flex: 1,
    gap: 2,
  },
  mapResultsHeaderActions: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 8,
    marginLeft: 12,
  },
  mapResultsTitle: {
    color: theme.textPrimary,
    fontSize: 18,
    fontWeight: '800',
  },
  mapResultsMeta: {
    color: theme.textMuted,
    fontSize: 12,
    fontWeight: '700',
  },
  mapResultsList: {
    gap: 10,
    paddingRight: 4,
  },
  mapResultsScroll: {
    minHeight: 0,
  },
  mapResultsMoreButton: {
    alignItems: 'center',
    alignSelf: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.borderStrong,
    borderRadius: 999,
    borderWidth: 1,
    paddingHorizontal: 14,
    paddingVertical: 10,
  },
  mapResultsMoreButtonLoadingContent: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 8,
  },
  mapResultsMoreButtonText: {
    color: theme.accentBlue,
    fontSize: 13,
    fontWeight: '800',
  },
  mapResultsCollapseButton: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 12,
    borderWidth: 1,
    height: 32,
    justifyContent: 'center',
    width: 32,
  },
  mapResultsChevronIcon: {
    height: 14,
    position: 'relative',
    width: 16,
  },
  mapResultsChevronLine: {
    backgroundColor: theme.textPrimary,
    borderRadius: 999,
    height: 2.5,
    position: 'absolute',
    top: 6,
    width: 8,
  },
  mapResultsChevronLineLeft: {
    left: 0.5,
    transformOrigin: 'right center',
  },
  mapResultsChevronLineRight: {
    right: 0.5,
    transformOrigin: 'left center',
  },
  mapResultsContent: {
    gap: 12,
    minHeight: 0,
    overflow: 'hidden',
  },
  mapResultRow: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 18,
    borderWidth: 1,
    flexDirection: 'row',
    gap: 12,
    justifyContent: 'space-between',
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  mapResultCopy: {
    flex: 1,
    gap: 4,
  },
  mapResultTitle: {
    color: theme.textPrimary,
    fontSize: 15,
    fontWeight: '800',
  },
  mapResultMeta: {
    color: theme.textSecondary,
    fontSize: 12,
    lineHeight: 17,
  },
  mapResultAction: {
    color: theme.accentBlue,
    fontSize: 12,
    fontWeight: '800',
  },
  mapResultEmptyText: {
    color: theme.textSecondary,
    fontSize: 13,
    lineHeight: 18,
  },
  mapPreviewHeader: {
    alignItems: 'flex-start',
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  mapPreviewCopy: {
    flex: 1,
    gap: 4,
    paddingRight: 12,
  },
  mapPreviewTitle: {
    color: theme.textPrimary,
    fontSize: 19,
    fontWeight: '800',
  },
  mapPreviewTitleLandscape: {
    fontSize: 16,
  },
  mapPreviewMeta: {
    color: theme.textSecondary,
    fontSize: 14,
    fontWeight: '600',
  },
  mapPreviewMetaLandscape: {
    fontSize: 12,
  },
  mapPreviewActions: {
    flexDirection: 'row',
    gap: 8,
  },
  mapPreviewDetails: {
    gap: 4,
    marginTop: -2,
  },
  mapPreviewDetailText: {
    color: theme.textSecondary,
    fontSize: 13,
    lineHeight: 18,
  },
  mapPreviewDetailTextLandscape: {
    fontSize: 12,
    lineHeight: 16,
  },
  mapPreviewIconButton: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.borderStrong,
    borderRadius: 999,
    borderWidth: 1,
    height: 36,
    justifyContent: 'center',
    width: 36,
  },
  mapPreviewIconButtonLandscape: {
    height: 30,
    width: 30,
  },
  mapPreviewIconText: {
    color: theme.textPrimary,
    fontSize: 18,
    fontWeight: '800',
  },
  mapPreviewIconTextLandscape: {
    fontSize: 15,
  },
  mapPreviewGallery: {
    gap: 12,
  },
  mapPreviewGalleryLandscape: {
    gap: 8,
  },
  mapPreviewImage: {
    backgroundColor: theme.bgMuted,
    borderRadius: 18,
    height: 180,
    width: 250,
  },
  mapPreviewImageLandscape: {
    borderRadius: 14,
    height: 110,
    width: 180,
  },
  mapPreviewEmptyState: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderWidth: 1,
    borderRadius: 18,
    justifyContent: 'center',
    minHeight: 140,
    paddingHorizontal: 16,
    paddingVertical: 20,
  },
  mapPreviewEmptyStateLandscape: {
    minHeight: 92,
    paddingHorizontal: 12,
    paddingVertical: 14,
  },
  mapPreviewEmptyText: {
    color: theme.textSecondary,
    fontSize: 13,
    lineHeight: 18,
    textAlign: 'center',
  },
  mapPreviewEmptyTextLandscape: {
    fontSize: 12,
    lineHeight: 16,
  },
  mapAppTitle: {
    color: theme.textPrimary,
    fontSize: 24,
    fontWeight: '800',
  },
  mapOverlayText: {
    color: theme.textSecondary,
    fontSize: 13,
    lineHeight: 18,
  },
  currentHappyHoursLayer: {
    alignItems: 'flex-start',
    left: 0,
    position: 'absolute',
    right: 0,
    zIndex: 80,
  },
  currentHappyHoursPanel: {
    alignSelf: 'flex-start',
    backgroundColor: 'rgba(18, 22, 30, 0.98)',
    borderColor: theme.borderStrong,
    borderRadius: 22,
    borderWidth: 1,
    marginBottom: 8,
    maxHeight: 360,
    maxWidth: 420,
    overflow: 'hidden',
    shadowColor: theme.shadow,
    shadowOffset: { width: 0, height: 12 },
    shadowOpacity: 0.24,
    shadowRadius: 24,
    width: '100%',
  },
  currentHappyHoursPanelLight: {
    backgroundColor: 'rgba(248, 251, 255, 0.98)',
    borderColor: theme.surfaceLightBorder,
  },
  currentHappyHoursPanelHeader: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
    paddingHorizontal: 14,
    paddingTop: 13,
  },
  currentHappyHoursPanelHeading: {
    flex: 1,
    gap: 2,
  },
  currentHappyHoursLiveHeading: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 7,
  },
  currentHappyHoursLiveDot: {
    backgroundColor: theme.accentStrong,
    borderRadius: 999,
    height: 8,
    width: 8,
  },
  currentHappyHoursPanelTitle: {
    color: theme.textPrimary,
    fontSize: 16,
    fontWeight: '800',
  },
  currentHappyHoursPanelMeta: {
    color: theme.textMuted,
    fontSize: 12,
    fontWeight: '700',
  },
  currentHappyHoursCloseButton: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 999,
    borderWidth: 1,
    height: 44,
    justifyContent: 'center',
    marginLeft: 10,
    width: 44,
  },
  currentHappyHoursCloseButtonLight: {
    backgroundColor: theme.surfaceLight,
    borderColor: theme.surfaceLightBorder,
  },
  currentHappyHoursList: {
    gap: 8,
    padding: 10,
  },
  currentHappyHoursScroll: {
    maxHeight: 284,
  },
  currentHappyHoursRow: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 16,
    borderWidth: 1,
    flexDirection: 'row',
    gap: 10,
    minHeight: 68,
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  currentHappyHoursRowLight: {
    backgroundColor: theme.surfaceLightAlt,
    borderColor: theme.surfaceLightBorder,
  },
  currentHappyHoursRowCopy: {
    flex: 1,
    gap: 2,
    minWidth: 0,
  },
  currentHappyHoursRowTitle: {
    color: theme.textPrimary,
    fontSize: 15,
    fontWeight: '800',
  },
  currentHappyHoursRowMeta: {
    color: theme.textMuted,
    fontSize: 11,
    fontWeight: '700',
  },
  currentHappyHoursDealText: {
    color: theme.textSecondary,
    fontSize: 12,
    lineHeight: 16,
  },
  currentHappyHoursDealTitle: {
    color: theme.textPrimary,
    fontSize: 12,
    fontWeight: '800',
  },
  currentHappyHoursTrigger: {
    alignItems: 'center',
    backgroundColor: 'rgba(18, 22, 30, 0.98)',
    borderColor: theme.borderStrong,
    borderRadius: 999,
    borderWidth: 1,
    flexDirection: 'row',
    gap: 9,
    minHeight: 52,
    paddingHorizontal: 14,
    paddingVertical: 7,
    shadowColor: theme.shadow,
    shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.22,
    shadowRadius: 18,
    elevation: 12,
  },
  currentHappyHoursTriggerLight: {
    backgroundColor: 'rgba(248, 251, 255, 0.98)',
    borderColor: theme.surfaceLightBorder,
  },
  currentHappyHoursTriggerCopy: {
    gap: 1,
  },
  currentHappyHoursTriggerLabelRow: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 6,
  },
  currentHappyHoursTriggerTitle: {
    color: theme.textPrimary,
    fontSize: 14,
    fontWeight: '800',
  },
  currentHappyHoursTriggerMeta: {
    color: theme.textMuted,
    fontSize: 11,
    fontWeight: '700',
  },
  currentHappyHoursLightText: {
    color: theme.textDark,
  },
  currentHappyHoursLightMutedText: {
    color: theme.textDarkSoft,
  },
  mapOfflineBanner: {
    alignItems: 'center',
    alignSelf: 'center',
    backgroundColor: 'rgba(35, 31, 22, 0.94)',
    borderColor: 'rgba(247, 200, 115, 0.45)',
    borderRadius: 999,
    borderWidth: 1,
    flexDirection: 'row',
    gap: 7,
    paddingHorizontal: 13,
    paddingVertical: 8,
  },
  mapOfflineBannerText: {
    color: '#f7c873',
    fontSize: 12,
    fontWeight: '700',
  },
  mapLastKnownLocationText: {
    color: '#f7c873',
    fontSize: 13,
    fontWeight: '700',
  },
  searchRow: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 8,
  },
  searchRowLandscape: {
    alignItems: 'stretch',
    gap: 6,
  },
  searchInputShell: {
    alignItems: 'center',
    backgroundColor: '#1b1d23',
    borderColor: 'rgba(255, 255, 255, 0.08)',
    borderRadius: 20,
    borderWidth: 1,
    shadowColor: '#000000',
    shadowOffset: { width: 0, height: 10 },
    shadowOpacity: 0.18,
    shadowRadius: 18,
    elevation: 10,
    flex: 1,
    flexDirection: 'row',
    gap: 10,
    minHeight: 54,
    paddingHorizontal: 12,
  },
  searchInputShellLandscape: {
    minHeight: 46,
    paddingHorizontal: 10,
  },
  searchInputShellOverlay: {
    backgroundColor: 'rgba(24, 25, 31, 0.94)',
  },
  searchInputShellCurtain: {
    minHeight: 54,
  },
  searchCurtainToggleInline: {
    alignItems: 'center',
    bottom: -4,
    height: 20,
    justifyContent: 'center',
    left: '50%',
    marginLeft: -16,
    position: 'absolute',
    width: 32,
    zIndex: 12,
  },
  searchCurtainChevronIcon: {
    height: 16,
  },
  searchCurtainChevronLine: {
    backgroundColor: '#f5f7fb',
  },
  searchInputIconBadge: {
    alignItems: 'center',
    backgroundColor: '#2a2d36',
    borderColor: 'rgba(255, 255, 255, 0.06)',
    borderRadius: 999,
    borderWidth: 1,
    height: 30,
    justifyContent: 'center',
    width: 30,
  },
  searchInput: {
    color: '#f5f7fb',
    flex: 1,
    fontSize: 15,
    fontWeight: '500',
    letterSpacing: 0,
    paddingVertical: 0,
  },
  searchClearButton: {
    alignItems: 'center',
    height: 28,
    justifyContent: 'center',
    width: 28,
  },
  filtersToggleButton: {
    alignItems: 'center',
    backgroundColor: '#1f2026',
    borderColor: 'rgba(255, 255, 255, 0.08)',
    borderRadius: 18,
    borderWidth: 1,
    flexDirection: 'row',
    gap: 6,
    justifyContent: 'center',
    minHeight: 44,
    paddingHorizontal: 12,
  },
  filtersToggleButtonInline: {
    minHeight: 44,
    paddingHorizontal: 12,
  },
  filtersToggleButtonPressable: {
    borderRadius: 18,
  },
  filtersToggleButtonLandscape: {
    minHeight: 44,
    paddingHorizontal: 12,
  },
  filtersToggleButtonActive: {
    backgroundColor: theme.accentSoft,
    borderColor: theme.borderStrong,
  },
  filtersToggleText: {
    color: '#f5f7fb',
    fontSize: 14,
    fontWeight: '600',
  },
  filtersToggleTextInline: {
    fontSize: 14,
  },
  filtersToggleTextActive: {
    color: '#ffffff',
  },
  filtersToggleIconWrap: {
    alignItems: 'center',
    justifyContent: 'center',
  },
  browseStatsRow: {
    alignItems: 'flex-start',
    flexDirection: 'row',
    gap: 8,
    paddingTop: 2,
    justifyContent: 'space-between',
  },
  browseStatsRowLandscape: {
    gap: 6,
  },
  browseStatsText: {
    color: '#f5f7fb',
    fontSize: 13,
    fontWeight: '800',
  },
  browseStatsSubtleText: {
    color: '#8c92a3',
    flex: 1,
    fontSize: 12,
    fontWeight: '600',
    textAlign: 'right',
  },
  filtersPanel: {
    borderTopColor: 'rgba(255, 255, 255, 0.08)',
    borderTopWidth: 1,
    gap: 12,
    marginTop: 6,
    minHeight: 0,
  },
  filtersPanelLandscape: {
    gap: 8,
    paddingTop: 8,
  },
  filtersPanelScrollContent: {
    gap: 12,
    paddingTop: 5,
    paddingBottom: 4,
    paddingRight: 4,
  },
  browseSectionHeaderRow: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  browseSectionTitle: {
    color: '#b7bdca',
    fontSize: 14,
    fontWeight: '800',
  },
  browseSectionMeta: {
    color: '#7f8597',
    fontSize: 12,
    fontWeight: '600',
  },
  browseSectionAction: {
    color: '#ff7a68',
    fontSize: 12,
    fontWeight: '700',
  },
  reloadButton: {
    alignItems: 'center',
    backgroundColor: '#2a2d36',
    borderColor: 'rgba(255, 255, 255, 0.08)',
    borderRadius: 16,
    borderWidth: 1,
    height: 44,
    justifyContent: 'center',
    width: 44,
  },
  secondaryToolbarButton: {
    backgroundColor: '#1f2026',
    borderColor: 'rgba(255, 255, 255, 0.08)',
    borderRadius: 16,
    borderWidth: 1,
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  secondaryToolbarButtonText: {
    color: '#f5f7fb',
    fontSize: 13,
    fontWeight: '700',
  },
  toolbarArrowButton: {
    alignItems: 'center',
    backgroundColor: '#1f2026',
    borderColor: 'rgba(255, 255, 255, 0.08)',
    borderRadius: 16,
    borderWidth: 1,
    height: 44,
    justifyContent: 'center',
    width: 44,
  },
  toolbarArrowButtonText: {
    color: '#f5f7fb',
    fontSize: 22,
    fontWeight: '700',
  },
  reloadButtonText: {
    color: '#f4fffe',
    fontSize: 22,
    fontWeight: '700',
  },
  mapThemeToggleButton: {
    alignItems: 'center',
    borderRadius: 16,
    justifyContent: 'center',
    height: 44,
    overflow: 'hidden',
    width: 44,
  },
  mapThemeToggleButtonFill: {
    alignItems: 'center',
    backgroundColor: '#1f2026',
    borderColor: 'rgba(255, 255, 255, 0.08)',
    borderRadius: 16,
    borderWidth: 1,
    height: '100%',
    justifyContent: 'center',
    width: '100%',
  },
  mapThemeToggleButtonActive: {
    backgroundColor: theme.accentSoft,
    borderColor: theme.borderStrong,
  },
  mapThemeToggleButtonText: {
    color: '#d3d8e3',
    fontSize: 18,
    fontWeight: '800',
  },
  mapThemeToggleButtonTextLayer: {
    position: 'absolute',
  },
  mapThemeToggleButtonTextActive: {
    color: '#f5f7fb',
  },
  filterRow: {
    alignItems: 'flex-start',
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 8,
  },
  modeRow: {
    flexDirection: 'row',
    gap: 10,
  },
  modeSwitcherDock: {
    alignItems: 'flex-start',
    marginTop: 10,
  },
  modeSwitcherDockOverlay: {
    alignSelf: 'flex-start',
    marginTop: 8,
  },
  modeSwitcherTrack: {
    backgroundColor: theme.bgElevated,
    borderColor: theme.borderStrong,
    borderRadius: 18,
    borderWidth: 1,
    flexDirection: 'row',
    height: 44,
    overflow: 'hidden',
    padding: 4,
    position: 'relative',
  },
  modeSwitcherTrackOverlay: {
    backgroundColor: 'rgba(22, 26, 34, 0.98)',
  },
  modeLockBadge: {
    alignItems: 'center',
    backgroundColor: theme.bgElevated,
    borderColor: theme.borderStrong,
    borderRadius: 999,
    borderWidth: 1,
    justifyContent: 'center',
    minHeight: 44,
    minWidth: 116,
    paddingHorizontal: 16,
  },
  modeLockBadgeText: {
    color: theme.textPrimary,
    fontSize: 13,
    fontWeight: '800',
  },
  modeSwitcherThumb: {
    backgroundColor: theme.accent,
    borderRadius: 999,
    bottom: 4,
    left: 4,
    position: 'absolute',
    top: 4,
  },
  modeSwitchOption: {
    alignItems: 'center',
    justifyContent: 'center',
    zIndex: 1,
  },
  modeSwitchOptionText: {
    color: theme.textSecondary,
    fontSize: 13,
    fontWeight: '700',
  },
  modeSwitchOptionTextActive: {
    color: '#f4fffe',
  },
  filterChip: {
    alignSelf: 'flex-start',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 999,
    borderWidth: 1.5,
    overflow: 'hidden',
    paddingHorizontal: 14,
    paddingVertical: 10,
  },
  filterChipActive: {
    backgroundColor: theme.accentSoft,
    borderColor: theme.borderStrong,
  },
  filterChipText: {
    color: theme.textPrimary,
    fontSize: 13,
    fontWeight: '600',
  },
  filterChipTextActive: {
    color: theme.textPrimary,
  },
  overlayChip: {
    alignSelf: 'flex-start',
    backgroundColor: 'rgba(22, 26, 34, 0.98)',
    borderColor: theme.border,
    borderRadius: 999,
    borderWidth: 1.5,
    overflow: 'hidden',
    paddingHorizontal: 14,
    paddingVertical: 10,
  },
  overlayChipActive: {
    backgroundColor: theme.accentSoft,
    borderColor: theme.borderStrong,
  },
  overlayChipText: {
    color: theme.textPrimary,
    fontSize: 13,
    fontWeight: '600',
  },
  overlayChipTextActive: {
    color: theme.textPrimary,
  },
  venueFilterRow: {
    alignItems: 'center',
    gap: 8,
    paddingRight: 16,
  },
  venueFilterChip: {
    alignItems: 'center',
    alignSelf: 'flex-start',
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 999,
    borderWidth: 1.5,
    flexDirection: 'row',
    gap: 8,
    overflow: 'hidden',
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  venueFilterChipActive: {
    backgroundColor: theme.accentSoft,
    borderColor: theme.borderStrong,
  },
  mapMarkerMotion: {
    alignItems: 'center',
    height: 34,
    justifyContent: 'center',
    width: 32,
  },
  mapMarker: {
    alignItems: 'center',
    height: 34,
    justifyContent: 'center',
    width: 32,
  },
  mapMarkerLayer: {
    left: 0,
    position: 'absolute',
    top: 0,
  },
  mapMarkerCore: {
    borderRadius: 999,
    borderWidth: 1.5,
    position: 'absolute',
  },
  mapMarkerActive: {
    elevation: 6,
    shadowColor: '#1f160f',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.32,
    shadowRadius: 5,
  },
  mapMarkerIcon: {
    alignItems: 'center',
    height: 16,
    justifyContent: 'center',
    left: 0,
    position: 'absolute',
    top: 5,
    width: 32,
  },
  mapMarkerIconGlyph: {
    includeFontPadding: false,
    textAlign: 'center',
  },
  mapMarkerHighlight: {
    backgroundColor: 'rgba(255, 250, 244, 0.78)',
    borderRadius: 999,
    height: 2,
    left: 9,
    position: 'absolute',
    top: 8,
    transform: [{ rotate: '-28deg' }],
    width: 6,
  },
  listContent: {
    gap: 12,
    paddingBottom: 142,
  },
  listContentLandscape: {
    paddingBottom: 148,
  },
  placeCardColumn: {
    gap: 12,
  },
  placeCard: {
    backgroundColor: theme.bgElevated,
    borderColor: theme.border,
    borderRadius: 22,
    borderWidth: 1,
    padding: 18,
  },
  placeCardImage: {
    borderRadius: 16,
    height: 168,
    marginBottom: 14,
    width: '100%',
  },
  placeCardLandscape: {
    flex: 1,
  },
  placeCity: {
    color: theme.accentStrong,
    fontSize: 12,
    fontWeight: '700',
    letterSpacing: 0.8,
    textTransform: 'uppercase',
  },
  placeTitle: {
    color: theme.textPrimary,
    fontSize: 20,
    fontWeight: '800',
    marginTop: 4,
  },
  placeMeta: {
    color: theme.textSecondary,
    fontSize: 14,
    marginTop: 4,
  },
  placeAddress: {
    color: theme.textMuted,
    fontSize: 13,
    marginTop: 8,
  },
  centerState: {
    alignItems: 'center',
    flex: 1,
    justifyContent: 'center',
    paddingVertical: 40,
  },
  centerStateText: {
    color: '#6f5947',
    marginTop: 10,
  },
} as const;

