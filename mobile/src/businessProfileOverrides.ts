...
export function buildNormalizedOperatingHourOverrides(overrides: BusinessOperatingHourOverride[]) {
  return overrides
    .map((override) => ({
      ...
      open_time: override.open_time.trim(),
      close_time: override.close_time.trim(),
    }))
    .filter((override) => override.open_time && override.close_time)
    .sort((left, right) => left.weekday - right.weekday || left.open_time.localeCompare(right.open_time));
}
  ...
  return overrides
    .map((override) => ({
      ...
      happy_hours: override.happy_hours
        .map((window) => ({
          weekday: window.weekday,
          start_time: window.start_time.trim(),
          end_time: window.end_time.trim(),
          all_day: window.all_day,
        }))
...
export function formatOperatingHourGroups(operatingHours: BusinessOperatingHourOverride[] | OperatingHourWindow[]) {
  const groups = new Map<string, Array<BusinessOperatingHourOverride | OperatingHourWindow>>();
  operatingHours.forEach((operatingHour) => {
    const key = `${operatingHour.open_time}-${operatingHour.close_time}`;
    const currentGroup = groups.get(key) ?? [];
    currentGroup.push(operatingHour);
